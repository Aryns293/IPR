...
```mermaid
flowchart TD
    A[LIMITATIONS ON TRADEMARK RIGHTS]
    A --> B["Section 33\nAcquiescence"]
    A --> C["Section 34\nVested Rights"]
    A --> D["Section 35\nName/Address/\nDescription"]
    C --> E["Section 36\nDescriptive Words"]
...
```mermaid
flowchart TD
    A[A owns earlier trademark] --> B[B uses later trademark]
    B --> C[A knows + allows use]
    C --> D[5 continuous years]
    D --> E[No appropriate legal action]
    E --> F[Section 33 limitation]
```
...
```mermaid
flowchart TD
    A["B uses 'XYZ'\nSince 2015"] --> B["A registers 'XYZ'\nIn 2020"]
    B --> C[B = prior continuous user]
    C --> D[Section 34 protection]
```
...
```mermaid
flowchart TD
    A[Own Name / Address / Description] --> B[Bona fide use]
    B --> C[Section 35]
    C --> D["Registered owner cannot\nautomatically stop the use"]
```
...
```mermaid
flowchart TD
    A[Common / Descriptive Word] --> B[Ordinary use]
    B --> C["Trademark owner generally\ncannot monopolise it"]
    
    C --> D["BUT if word has\nAcquired Distinctiveness\nOR is Well-Known"]
    D --> E[Stronger trademark protection]
```
...
```mermaid
flowchart LR
    A["33 → SILENCE → 5 YEARS"]
    B["34 → PRIOR USE → USED FIRST"]
    C["35 → NAME → OWN IDENTITY"]
    D["36 → DESCRIPTION → COMMON WORD"]
...
## 🧠 Ultimate Mindmap: Trademark Limitations (Sec 33-36)
```mermaid
mindmap
  root((Limitations))
