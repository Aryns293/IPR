...
```mermaid
flowchart TD
    A[1624] --> B[Statute of Monopolies]
    B --> C[Foundation of modern patent principles]
    
    D[1710] --> E[Statute of Anne]
    E --> F[Early modern copyright law]
    
    G[1883] --> H[Paris Convention]
    H --> I[International industrial property protection]
    
    J[1886] --> K[Berne Convention]
    K --> L[International copyright protection]
    
    M[1967] --> N[WIPO Convention]
    N --> O[Establishment of WIPO]
    
    P[1974] --> Q[WIPO becomes UN specialized agency]
    
    R[1995] --> S[TRIPS Agreement]
    S --> T[Minimum international IPR standards]
...
```mermaid
flowchart TD
    A[New invention] --> B[Legal patent protection]
    B --> C[Inventor gets exclusive rights]
    C --> D[Others cannot commercially exploit it]
    D --> E[Inventor gets incentive to innovate]
```
...
```mermaid
flowchart TD
    A[You write book] --> B[Someone copies it]
    B --> C[Someone else publishes it]
    C --> D[You have little control]
```
...
```mermaid
flowchart TD
    A[Indian applicant] --> B[Foreign country]
    B --> C[National Treatment]
    C --> D[Protected under the same applicable rules as that country's nationals]
```
...
```mermaid
flowchart TD
    A[1 Jan] --> B[Patent filed in India]
    B --> C[Priority period]
    C --> D[File in other member countries]
    D --> E[Claim priority from 1 Jan]
```
...
```mermaid
flowchart TD
    A[Create original story] --> B[Copyright arises]
    B --> C[No compulsory international registration required under Berne]
```
...
```mermaid
flowchart TD
    A[Indian author] --> B[Creates novel]
    B --> C[Copyright]
    C --> D[Protection in Berne member countries according to their copyright laws]
```
...
```mermaid
flowchart TD
    A[WIPO] --> B[IP treaties]
    A --> C[Cooperation]
    A --> D[IP systems]
    B --> E[International IP protection]
    C --> E
    D --> E
```
...
flowchart TD
    ...
    I --> J[GI Act]
    J --> K[2001]
    K --> L[Semiconductor Layout-Design Act]
    L --> M[2005]
    M --> N[Patent Amendment]
```
...
```mermaid
flowchart TD
    A[The product itself is protected]
```
...
```mermaid
flowchart TD
    A[Engineer designs chip layout] --> B[Original layout-design]
    B --> C[Legal protection]
    C --> D[Unauthorized copying restricted]
```
...
```mermaid
flowchart TD
    A[1970 Patent Act] --> B[Certain areas such as pharma]
    B --> C[Process patents]
    C --> D[Product patents excluded]
```
