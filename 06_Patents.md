...
```mermaid
flowchart TD
    A[INVENTOR] -->|Creates an invention| B[PATENT OFFICE]
    B -->|Examines application| C[Patentability requirements]
    
    C --> D[Novelty]
    C --> E[Inventive Step]
    C --> F[Industrial Applicability]
...
```mermaid
flowchart TD
    A[Company] -->|Uses invention itself| B[ ]
    A --> C[Licenses technology]
    C --> D[Other companies]
    D --> E[Licensing income]
```
...
```mermaid
flowchart TD
    A[Existing battery] --> B[New technical product]
    B --> C[Possible product patent]
```
...
```mermaid
flowchart TD
    A[Battery A] --> B[Existing technology]
```
Inventor develops:
```mermaid
flowchart TD
...
flowchart TD
    ...
    A --> B[NOVELTY]
    A --> C[INVENTIVE STEP]
    A --> D[INDUSTRIAL APPLICABILITY]
    A --> E[NOT EXCLUDED SUBJECT MATTER]
```
...
flowchart TD
    ...
    H[Compare invention]
    H --> I[Already disclosed]
    I --> J[Novelty problem]
    
    H --> K[Not previously disclosed]
    K --> L[May satisfy novelty]
```
...
```mermaid
flowchart TD
    A[NEW INVENTION] --> B{Is it NOVEL?}
    B -->|YES| C{Is it INVENTIVE?}
    C -->|YES| D{INDUSTRIALLY APPLICABLE?}
    D -->|YES| E{EXCLUDED SUBJECT?}
    E -->|NO| F[MAY BE PATENTABLE]
```
...
```mermaid
flowchart TD
    A[NEW PRODUCT] --> B[Product patent]
    B --> C[Protects patented product]
```

* **Process Patent:** Protects the **method/process used to produce something**. Example: A new method of manufacturing a chemical.
```mermaid
flowchart TD
    A[NEW MANUFACTURING METHOD] --> B[Process patent]
    B --> C[Protects the process]
```
