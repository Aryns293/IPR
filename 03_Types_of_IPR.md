...
flowchart TD
    ...
    C --> F[TRADEMARK]
    D --> G[COPYRIGHT]
    E --> H[Technical invention]
    I[Product appearance] --> J[INDUSTRIAL DESIGN]
```
...
flowchart TD
    ...
    B --> E[License]
    E --> F[Assign]
    F --> G[Enforce against infringement]
```
...
```mermaid
flowchart TD
    A[Programmer] --> B[Software application]
    B --> C[Source Code]
    C --> D[Copyright Protection]
```
...
flowchart TD
    A[COPYRIGHT]
    B[Creation of eligible original work] --> C[Copyright generally arises]
    C --> D[Registration NOT required for copyright to exist]
```
...
flowchart TD
    A[PRODUCT]
    A --> B[Technical aspect]
    A --> C[Visual aspect]
    B --> D[PATENT]
    C --> E[INDUSTRIAL DESIGN]
```
...
flowchart TD
    ...
    A --> C[TRADEMARK]
    A --> D[COPYRIGHT]
    A --> E[INDUSTRIAL DESIGN]
    B --> F[Inventions]
    C --> G[Brand identity]
    D --> H[Creative works]
    E --> I[Product appearance]
```
