...
```mermaid
flowchart TD
    A[PATENT] --> B[EXCLUSIVE RIGHTS]
    B --> C[Subject to statutory limitations]
```
...
```mermaid
flowchart TD
    A[Original Owner] -->|Assignment| B[New Owner]
```

**Example from the slide**
```mermaid
flowchart TD
    A[Researcher] -->|Assignment| B[Company]
```
...
```mermaid
flowchart TD
    A[Patent Owner] -->|License| B[Another Party]
    B --> C[Can use the patent]
```
But:
...
```mermaid
flowchart TD
    A[Patent Owner] -->|Voluntarily grants| B[Licensee]
```
* **Compulsory:**
```mermaid
flowchart TD
...
```mermaid
flowchart LR
    A[PATENT RIGHTS] <--> B[PUBLIC INTEREST]
```
...
```mermaid
flowchart TD
    A[Request] --> B[Controller examines]
    B --> C[Interested persons can oppose]
    C --> D[Decision according to Act]
```
