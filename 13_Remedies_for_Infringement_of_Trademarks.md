...
```mermaid
flowchart TD
    A[Infringement] --> B["Owner's exclusive rights affected"]
    B --> C[Business suffers loss]
    C --> D[Claim for damages]
```
...
```mermaid
flowchart TD
    A[Suspected infringement] --> B[Risk of evidence/goods being removed]
    B --> C[Court order]
    C --> D[Local Commissioner]
    D --> E[Secure / seal relevant goods]
```
...
```mermaid
flowchart TD
    A["Defendant's assets"] --> B["Risk of removal/disposal"]
    B --> C[Mareva Injunction]
    C --> D[Assets frozen]
    D --> E[Future judgment protected]
```
...
```mermaid
flowchart TD
    A[Unknown infringer] --> B[Future infringement feared]
    B --> C[John Doe Order]
    C --> D[Unknown person restrained]
```
...
```mermaid
flowchart TD
    A[Unknown wrongdoer] --> B[Third party has information]
    B --> C[Norwich Pharmacal Order]
    C --> D[Disclosure of relevant information]
    D --> E["Wrongdoer identified / case strengthened"]
```
...
```mermaid
flowchart TD
    A["103 → FALSE MARK"] --> B["104 → SELL FALSE MARK"]
    B --> C["105 → REPEAT"]
    C --> D["106 → REMOVE GOODS"]
    D --> E["107 → FALSE REGISTRATION"]
    E --> F["108 → FALSE OFFICE"]
    F --> G["109 → FALSE REGISTER"]
```
