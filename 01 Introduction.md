...
```mermaid
flowchart TD
    A[A invents new technology] --> B[Intellectual Property]
    B --> C[Patent protection]
    C --> D[A gets legal rights]
    D --> E[Others cannot commercially use the protected invention without authorization]
```
...
```mermaid
flowchart TD
    A[Software code] --> B[No physical object itself]
    B --> C[But can have huge economic value]
```
...
```mermaid
flowchart TD
    A[Patent in India] --> B[Protection under Indian patent law]
    C[Patent in USA] --> D[Protection under US patent law]
```
...
```mermaid
flowchart TD
    A[Company A] -- License --> B[Company B]
    B --> C[Uses software under agreed terms]
```
...
```mermaid
flowchart TD
    A[Unauthorized use] --> B[Possible infringement]
    B --> C[Legal remedies]
```
...
```mermaid
flowchart TD
    A[Patent owner] --> B[Licenses technology]
    B --> C[Another company uses it]
    C --> D[Royalty paid]
    D --> E[Income to IP owner]
```
...
```mermaid
flowchart TD
    A[New battery invention] --> B[Patent]
...
```mermaid
flowchart TD
    A[Program source code] --> B[Copyright]
```
...
```mermaid
flowchart TD
    A[Product] --> B[Unique visual appearance]
    B --> C[Industrial Design protection]
```
...
```mermaid
flowchart TD
    A[Specific geographical region] --> B[Darjeeling]
    B --> C[Darjeeling Tea]
    C --> D[GI]
```
...
```mermaid
flowchart TD
    A[Plant breeding] --> B[New plant variety]
    B --> C[Protection under applicable plant variety law]
```
...
```mermaid
flowchart TD
    A[Original digital work] --> B[Can be copied quickly]
    B --> C[Can be distributed worldwide]
    C --> D[Unauthorized copies may spread]
    D --> E[IP protection becomes important]
```

For example, suppose someone creates a paid software application. Without appropriate legal protection:
```mermaid
flowchart TD
    A[Original Software] --> B[Someone copies it]
    B --> C[Uploads it elsewhere]
    C --> D[Thousands download it]
    D --> E[Creator may lose revenue]
```
...
flowchart TD
    ...
    C --> H[Creative works]
    D --> I[Brand]
    E --> J[Appearance of product]
    F --> K[Geographic origin]
    
    A --> L[TRADE SECRET]
    A --> M[PLANT VARIETY PROTECTION]
    L --> N[Confidential information]
    M --> O[New plant varieties]
```
