...
```mermaid
flowchart TD
    A[Creator's Rights] <--> B[Public Interest]
    B <--> C[Innovation]
    C <--> D[Economic Development]
```
...
```mermaid
flowchart TD
    A[Company develops technology]
    A --> B[Patent]
    B --> C[License]
    C --> D[Other company uses technology]
    D --> E[Royalty payment]
```
...
flowchart TD
    ...
    H --> I[Technology Transfer]
    I --> J[Startup Development]
    J --> K[Economic Development]
```
